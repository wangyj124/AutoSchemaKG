...
neo4j_uri = "bolt://localhost:8612" # use bolt port for driver connection
user = "neo4j"
password = "admin2024"
keyword = 'dulce' # can be wiki or pes2o  # keyword to identify the cc_en dataset
driver = GraphDatabase.driver(neo4j_uri, auth=(user, password))

text_index = faiss.read_index(f"import/Dulce/vector_index/text_nodes_Dulce_from_json_with_emb_non_norm.index", faiss.IO_FLAG_MMAP)
node_index = faiss.read_index(f"import/Dulce/vector_index/triple_nodes_Dulce_from_json_with_emb_non_norm.index", faiss.IO_FLAG_MMAP)

# setup logger
date_time = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")    
log_file_path = f'./log/LargeKGRAG_dulce.log'
