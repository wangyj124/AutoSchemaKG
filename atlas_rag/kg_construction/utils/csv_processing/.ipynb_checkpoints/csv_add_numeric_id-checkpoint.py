...
def add_csv_columns(node_csv, edge_csv, text_csv, node_with_numeric_id, edge_with_numeric_id, text_with_numeric_id):
    with open(node_csv, encoding='utf-8') as infile, open(node_with_numeric_id, 'w', newline='', encoding='utf-8') as outfile:
        reader = csv.reader(infile)
        ...
        for row_number, row in tqdm(enumerate(reader), desc="Adding numeric ID"):
            ...
            writer.writerow(row)
    with open(edge_csv, encoding='utf-8') as infile, open(edge_with_numeric_id, 'w', newline='', encoding='utf-8') as outfile:
        reader = csv.reader(infile)
        writer = csv.writer(outfile)
        header = next(reader)
        print(header)
        ...
        for row_number, row in tqdm(enumerate(reader), desc="Adding numeric ID"):
            row.insert(label_index, row_number)  # Add numeric ID before ':LABEL'
            writer.writerow(row)
    with open(text_csv, encoding='utf-8') as infile, open(text_with_numeric_id, 'w', newline='', encoding='utf-8') as outfile:
        reader = csv.reader(infile)
        writer = csv.writer(outfile)
        header = next(reader)
        print(header)
